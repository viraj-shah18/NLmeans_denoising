...
def save_all(noisy_image, gauss_output, nl_output, save_name):
...
def print_metrics(gt_image, noisy_image, guass_output, nl_output, ):
    print("-"*100)
    print(" "*30+ "PSNR (all compared with true image) S&P  Noise")
    print("-"*100)
    ...
    print(f"MSE error after gaussian filter with predicted image= {mean_squared_error(guass_output, nl_output)}")
